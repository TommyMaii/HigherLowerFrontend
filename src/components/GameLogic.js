...
export default function GameLogic(gamesProp, gameMode = 'price') {
  ...
  onBeforeMount(async () => {
    try {
      const response = await fetch('https://higherlowerbackend.onrender.com/getHighscore');
  ...
  const pushHighscoreAfterLoss = async (newHighscore) => {
    try {
      const response = await fetch('https://higherlowerbackend.onrender.com/UpdateHighscore', {
        method: "PUT",
        headers: {
