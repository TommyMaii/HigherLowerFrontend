...
export default {
  ...
  setup() {
    ...
    onBeforeMount(async () => {
      try {
        const response = await fetch('https://higherlowerbackend.onrender.com/getGames')
        if (!response.ok) {
...
<template>
  <div v-if="games.length === 0">
    <h2>Loading... Refresh every 5 seconds if it's not loading, Render is taking its sweet time</h2>
